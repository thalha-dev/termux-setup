...
# NOTE: must NOT live in /tmp — with --shared-tmp, Termux's $PREFIX/tmp is
# bound OVER the container's /tmp, hiding anything we place at rootfs/tmp.
# /opt is not bound by proot-distro, so it stays visible inside.
...
# Liveness check: the session must have started writing its log by now.
log "Waiting for the session to come up..."
started=0
for _ in $(seq 1 15); do
  if [ -s "$ROOTFS/opt/xd-session.log" ]; then started=1; break; fi
  sleep 1
done
if [ "$started" = 0 ]; then
  die "session never started (no log). Debug it live:
  proot-distro login $CONTAINER --user $UBU_USER --shared-tmp --shared-x11 -- /bin/bash /opt/xd-session.sh"
fi
tail -n 5 "$ROOTFS/opt/xd-session.log" || true
...
Still black?  Read the session log and send it:
  cat $PREFIX/var/lib/proot-distro/containers/${CONTAINER}/rootfs/opt/xd-session.log
